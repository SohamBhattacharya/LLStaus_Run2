...
def main() :
    ...
    fname_in = "output_stau_lifetime-reweighting_study.root"
    ...
    d_stau_config["MStau-250_ctau-1mm"] = [
        "MStau-250_ctau-1mm",
        "MStau-250_ctau-1mm-from-5mm",
        "MStau-250_ctau-1mm-from-10mm",
        "MStau-250_ctau-1mm-from-50mm",
        #"MStau-250_ctau-1mm-from-100mm",
    ]
    
    d_stau_config["MStau-250_ctau-5mm"] = [
        "MStau-250_ctau-5mm",
        "MStau-250_ctau-5mm-from-10mm",
        "MStau-250_ctau-5mm-from-50mm",
        #"MStau-250_ctau-5mm-from-100mm",
    ]
    
    d_stau_config["MStau-250_ctau-10mm"] = [
        "MStau-250_ctau-10mm",
        "MStau-250_ctau-10mm-from-50mm",
        #"MStau-250_ctau-10mm-from-100mm",
        "MStau-250_ctau-20mm-from-50mm",
        "MStau-250_ctau-30mm-from-50mm",
        "MStau-250_ctau-40mm-from-50mm",
    ]
    
    d_stau_config["MStau-250_ctau-50mm"] = [
        "MStau-250_ctau-50mm",
        "MStau-250_ctau-50mm-from-100mm",
        "MStau-250_ctau-60mm-from-100mm",
        "MStau-250_ctau-75mm-from-100mm",
        "MStau-250_ctau-90mm-from-100mm",
    ]
    
    infile = ROOT.TFile.Open(fname_in)
    outdir = "output/stau_lifetime-reweighting_study"
        ...
        for stau_key, stau_config in d_stau_config.items() :
            ...
            for cfg_name in stau_config :
                ...
                if "from" in cfg_name :
                    ...
                    regexp = "MStau-(?P<mstau>\d+)_ctau-(?P<ctau>\w+)mm-from-(?P<ctaufrom>\w+)mm"
                
                rgx = re.compile(regexp)
                
                #stau_info = cmut.parse_stau_samplestring(
                #    s = cfg_name,
                #    regexp = regexp,
                #)
                ...
                ctau = 0.1*float(stau_info["ctau"].replace("p", "."))
                
                hist_title = f"#tilde{{#tau}}_{{1}}({mstau}), c#tau_{{0}}({ctau:0.1f} cm)"
                
                if "from" in cfg_name :
                    
                    ctaufrom = 0.1*float(stau_info["ctaufrom"].replace("p", "."))
                    hist_title = f"{hist_title}#Leftarrowc#tau_{{0}}({ctaufrom:0.1f} cm)"
